...
function mapExerciseFields(ex: any, unit: WeightUnit = 'kg') {
  ...
  const mapped = {
    ...
    perSideSeconds: ex.perSideSeconds ?? null,
    sideSwitchRestSeconds: ex.sideSwitchRestSeconds ?? null,
    unilateralCue: ex.unilateralCue ?? null,
...
function mapCooldownExercise(ex: any) {
  ...
  return {
    ...
    duracion: ex.duracion ?? (ex.durationSeconds ? `${ex.durationSeconds} seg` : undefined),
...
export function normalizeSession(raw: any, options: { weightUnit?: WeightUnit } = {}): GeneratedSession | null {
  ...
  if (Array.isArray(session.warmup)) {
    session.warmup = session.warmup.map((item: any) => {
      ...
      perSideSeconds: item.perSideSeconds ?? null,
      sideSwitchRestSeconds: item.sideSwitchRestSeconds ?? null,
      unilateralCue: item.unilateralCue ?? null,
      restAfterSeconds: item.restAfterSeconds ?? null,
      durationSeconds: item.durationSeconds ?? null,
      isRampSet: item.isRampSet === true,
      reps: item.reps,
